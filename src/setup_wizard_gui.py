...
class SetupWizardGUI(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.setup_data = {
            ...
            'max_tokens': tk.StringVar(value='2048'),  # Changed to StringVar to support 'auto'
    ...
    def show_llm_step(self):
        ...
        providers = [
            ("Google Gemini (Recommended - Fast & Affordable, very generous free tier)", "gemini"),
            ...
            try:
                ...
                if api_key:
                    try:
                        ...
                        fetched_models = loop.run_until_complete(openrouter.fetch_models_from_api())
                        loop.close()
                        if fetched_models:
    ...
    def generate_system_prompt(self):
        ...
        if not user_request.strip():
            messagebox.showwarning("No Input", "Please describe what you want your bot to do.")
            return
...
Respond with ONLY the system prompt text, nothing else."""
    ...
    def show_tools_step(self):
        ...
        ctk.CTkEntry(
            port_frame,
            textvariable=self.setup_data['dashboard_port'],
            width=100
        ).pack(side="left", padx=5)
    ...
    def show_screening_step(self):
        ...
        self.screening_policy_text.insert("1.0", self.setup_data['screening_policy'].get())
    ...
    def show_final_step(self):
        ...
        # Summary frame
        summary_frame = ctk.CTkScrollableFrame(self.content_frame, width=600, height=300)
        ...
        summary_items = [
            ...
            ("Web Search", "Enabled" if self.setup_data['enable_search'].get() else "Disabled"),
            ("Content Screening", "Enabled" if self.setup_data['enable_screening'].get() else "Disabled"),
            ("Dashboard", f"Enabled (port {self.setup_data['dashboard_port'].get()})" if self.setup_data['enable_dashboard'].get() else "Disabled"),
        ]
...
5. Copy and save your key"""
        ...
        help_text = help_texts.get(provider, "Please visit the provider's website for API key instructions.")
        ...
        if self.current_step == 0:
            ...
            if self.setup_data['enable_screening'].get():
                ...
                if self.setup_data['screening_action'].get() == 'escalate':
                    ...
                    if not channel_id:
                        messagebox.showerror("Error", "Please enter a moderation channel ID for escalation.")
                        return False
    ...
    def finish_setup(self):
        ...
        try:
            ...
            self.config_manager.set('llm.temperature', self.setup_data['temperature'].get())
            self.config_manager.set('llm.max_tokens', self.setup_data['max_tokens'].get())
            self.config_manager.set('llm.system_prompt', self.setup_data['system_prompt'].get())
            
            self.config_manager.set('tools.web_search.enabled', self.setup_data['enable_search'].get())
            if self.setup_data['enable_search'].get():
                self.config_manager.set('tools.web_search.default_provider', self.setup_data['search_provider'].get())
            ...
            self.config_manager.set('dashboard.enabled', self.setup_data['enable_dashboard'].get())
            self.config_manager.set('dashboard.port', self.setup_data['dashboard_port'].get())
            ...
            if self.setup_data['enable_screening'].get():
                ...
                if self.setup_data['screening_action'].get() == 'escalate':
                    self.config_manager.set('screening.channel_id', self.setup_data['screening_channel_id'].get())
